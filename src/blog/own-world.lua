document {
	...
	p([[i feel like i just want things to be mine. its hard to be comfortable in a 
		space where you do not know what is going on, or how things work. and the only 
		way to know how something works, is to have made it yourself, either literally 
		or an exact replica. this is hard, obviously. we live in an increasingly 
		complex world, its pretty much impossible to truly start from scratch and 
		become one with it, at least without some self deception and ignoring certain 
		parts (how electric rock do math). i get anxious if i dont know if something is 
		going to do something i dont want it to, since its my computer regardless so 
		the consequences are naturally mine to burden. it feels like this should be 
		easy to avoid, but every person implants expectations into their creations, so 
		its just unlucky when those conflict with mine. its going to happen regardless. 
		if there were no shortcuts taken, then it wouldnt have much of a use anyways. 
		so thus, the goal is to build tooling and worlds for yourself where the 
		shortcuts are always, exactly (or at least within certain bounds), what you 
		expect.
	]]),
